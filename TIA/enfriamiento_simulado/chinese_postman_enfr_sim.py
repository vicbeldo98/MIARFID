...
def crear_vecinos_greedy(s_actual, n_vecinos=5):
    vecinos = []
    while(len(vecinos) < n_vecinos):
        ...
        while not factible(individual) and len(individual) < max_len:
            ...
            for street in next_street_posib:
                if street in non_visited_streets and edges[street][2] < distance:
                    next = street
                    distance = edges[street][2]
...
def crear_vecinos_random(s_actual, n_vecinos=3):
    vecinos = []
    while(len(vecinos) < n_vecinos):
        # Crear soluciones greedys de todas las posiciones
        pos = random.choice(range(len(s_actual)))
...
t_inicial = 1000.0
k = 2  # cte entre 0 y 1 para actualizar la temperatura
...
while(len(vecinos) > 0 and not condicion_parada()):
    ...
    if(ganancia > 0):
        ...
        prob_accept = math.pow(math.e, ganancia / t)
        prob = random.random()
        if prob < prob_accept:
            s_actual = s_nuevo
