...
def crear_vecinos(s_actual):
    vecinos = []
    # como hago vecinos factibles a partir de una factible
    for i in s_actual:
        aux = s_actual[:i] + s_actual[i + 1:]
        if factible(aux):
            vecinos.append(aux)
    return vecinos
...
t_inicial = 1000
k = 0.5  # cte entre 0 y 1 para actualizar la temperatura
...
while(len(vecinos) > 0 and not condicion_parada()):
    ...
    ganancia = f_nuevo - f_actual  # TODO: QUÉ PASA SI ALGUNO ES MATH.INF
    if(ganancia < 0):
        ...
        if f_actual < f_mejor:
            ...
            k = math.pow(math.e, ganancia / t)
            if random.random() < k:
                s_actual = s_nuevo
    t = actualizar_temperatura(iteration, t)
    iteration += 1

print('Best solution: ' + str(s_mejor))
print('Fitness: ' + str(f_mejor))
